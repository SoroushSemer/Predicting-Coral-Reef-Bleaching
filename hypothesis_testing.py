...
#converts all temperature values to flosts
...
#converts the coral cover rankings to percentages
...
#pulls out only the temperature coral cover and depth data
...
#filters out the coral cover row
...
df = n - 2
df = sc.broadcast(df)
m = transposed_rdd.count()
m = sc.broadcast(m)
#gets p value from pearson correlation
...
#caculate the sample mean of the temperatures
def get_mean_n(row):
...
# bleach threshold value from research
bleach_threshold = 28.6111

#calculates the t value against the bleach threshold
def get_t_val(row):
...
#gets the p value from the t value using one tailed t-test
def get_p_val(row):
...
def convert_bleach_thresholds(row):
    ...
    return row
#filters pvalues below 0.05 for significant reefs
